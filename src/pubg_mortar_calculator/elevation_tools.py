...
class ElevationTools:
    @staticmethod
    def draw_point_position(
        image: np.ndarray,
        point: tuple[int, int],
        color: tuple[int, int, int] = (255, 0, 0),
    ) -> np.ndarray:
        x, y = point
        cv2.line(image, (0, y), (image.shape[1], y), color, 3)

        cv2.line(image, (x, 0), (x, image.shape[0]), color, 3)
        return image

    # Inspirated by: https://github.com/Ashimdude/mortarkiller_desktop
    @staticmethod
    def get_elevation(
        y0: int, y1: int, fov_deg: float, dist: float, aspect_ratio: float = 16 / 9
    ) -> float:
    ...
    def get_elevated_distance(distance: float, elevation: float) -> float:
        ...
        discriminant = (
            max_mortar_distance**2
            - 2 * distance * max_mortar_distance * tan_beta
            - distance**2
        )
        ...
        elevated_distance = (
            distance + tan_beta * (max_mortar_distance - sqrt_term)
        ) / (tan_beta**2 + 1)

        return elevated_distance

    @staticmethod
    def calculate_mortar_distance(distance: float, mortar_distances: list[int]) -> int:
        differences = []
        for mortar_distance in mortar_distances:
            differences.append(abs(mortar_distance - distance))
        distance = mortar_distances[differences.index(min(differences))]
        return distance
...
if __name__ == "__main__":
    ...
    print(
        f"Distance: {distance} Elevation: {elevation} Elevated distance: {elevated_distance}"
    )
